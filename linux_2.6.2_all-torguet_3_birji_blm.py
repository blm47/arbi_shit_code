...
ver = 'log_2.6.2'
account = 'Lasombra'
Withdrawal_fee_count = 100000 #Ожидаемое кол-во сделок перед выводом
...
timer_statistic = 0
while x < 1:
    time.sleep(0.12) #В целях экономия процессорного времени
...
# Получаем балансы: ___
        if balans_checker < 1:
            try:
                ...
                nowZEC =float(balanses['Exmo']['ZEC']) + float(balanses['Poloniex']['ZEC'])
                nowBCH =float(balanses['Exmo']['BCH']) + float(balanses['Poloniex']['BCH'])
                nowETH =float(balanses['Exmo']['ETH']) + float(balanses['Poloniex']['ETH'])
